...
public class PrometheusClient
{
    ...
    public PrometheusClient(PrometheusConnectorConfig config, JsonCodec<Map<String, Object>> metricCodec, TypeManager typeManager)
    {
        ...
        setupProxy(clientBuilder, config.getProxyHost(), config.getProxyPort());
    ...
    private Map<String, Object> fetchMetrics(@NotNull JsonCodec<Map<String, Object>> metricsCodec, URI metadataUri)
    {
        return metricsCodec.fromJson(fetchUri(metadataUri));
    }

    public byte[] fetchUri(@NotNull URI uri)
    {
    ...
    private Optional<String> getBearerAuthInfoFromFile(@NotNull Optional<File> bearerTokenFile)
    {
    ...
    private static void setupProxy(OkHttpClient.Builder clientBuilder, @NotNull Optional<String> proxyHost, Optional<Integer> proxyPort)
    {
        if (proxyHost.isPresent() && proxyPort.isPresent()) {
            Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(proxyHost.get(), proxyPort.get()));

            clientBuilder.proxy(proxy);
        }
    }

    private static void setupBasicAuth(OkHttpClient.Builder clientBuilder, @NotNull Optional<String> user, Optional<String> password)
    {
    ...
    private static void setupTokenAuth(OkHttpClient.Builder clientBuilder, @NotNull Optional<String> accessToken)
    {
        accessToken.ifPresent(token -> clientBuilder.addInterceptor(tokenAuth(token)));
    }

    private static @NotNull Interceptor basicAuth(String user, String password)
    {
    ...
    private static @NotNull Interceptor tokenAuth(String accessToken)
    {
